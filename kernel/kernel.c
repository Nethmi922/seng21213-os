...
static void cmd_colour(const char *args) {
    int foreground = parse_number(&args);
    int background = parse_number(&args);
    if (foreground < 0 || foreground > 15 || background < 0 || background > 15) {
        vga_puts("  Usage: colour <foreground 0-15> <background 0-15>\n");
        return;
    }
    vga_set_color((vga_color_t)foreground, (vga_color_t)background);
